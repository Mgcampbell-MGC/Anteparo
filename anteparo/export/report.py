...
def write_report(db, run_id, out_dir, counts):
    ...
    lines = [f"# ANTEPARO index — Phase 1 report", f"run `{run_id}` · {time.strftime('%Y-%m-%d %H:%MZ', time.gmtime())}", "",
             ...
             f"- closer workbook (ANTEPARO_CALL_SHEET.xlsx): trade creditors on CALL SHEET; banks/FIDCs/co-ops ({excl[0] or 0} companies by CNAE) on FINANCIAL CREDITORS; state-owned creditors on STATE-OWNED CREDITORS; public bodies ({excl[1] or 0}) and inactive companies ({excl[2] or 0}) on EXCLUDED (review); name pre-filter hits: {byname}",
